...
if(path === '/'){
    ...
    let mySession = sessions[hash.sessionId]   //从session的随机id里找已登录的email
    let email 
    if(mySession){
  ...
  } else if(path === '/sign_in' && method === 'GET'){   //登录
    ...
    readBody(request).then((body)=>{
      ...
      var users = fs.readFileSync('./db/users','utf-8')  //得到的字符串转换成json语言
        ...
        for(let i=0;i<users.length;i++){   //遍历数据库
          ...
          if(found){   //找有没有cookie
            let sessionId = Math.random()*1000000     //增加随机id
            sessions[sessionId] = {sign_in_email:email}
            response.setHeader('Set-Cookie', `sessionId=${sessionId};HttpOnly`)  //禁止js修改
            response.statusCode = 200
          }else{   //有cookie认证失败
